...
impl NeuralEvaluator {
    ...
    pub fn generate_and_save_random_boards(
        ...
        let db: TrainingDatabase = self.get_db()?;
        ...
        for i in 1..=count {
            ...
            let random_board: Board = game.generate_random_board();
            let board_sfen: String = random_board.to_string();
    ...
    pub fn update_records_with_random_games(
        ...
        let db: TrainingDatabase = self.get_db()?;
        let start_time: Instant = Instant::now();
        let records: Vec<(i64, String)> = db.read_records_for_update(max_records)?;
        ...
        let mut all_results: Vec<(i64, i32, i32, i32)> = Vec::new();

        for (id, board_sfen) in records {
            let board: Board = Board::from_sfen(board_sfen);
            let color: ColorType = if rand::random::<bool>() {
                ColorType::White
            } else {
                ColorType::Black
            };
            let game: Game = Game::from(board.clone(), 1, color, ColorType::None);

            let mcts_results: Vec<crate::mctsresult::MctsResult> =
                game.random_move_parallel(trials_per_record, num_threads);
            let white_wins: i32 = mcts_results.iter().map(|r| r.white_wins as i32).sum();
            let black_wins: i32 = mcts_results.iter().map(|r| r.black_wins as i32).sum();
            let total_games: i32 = mcts_results.iter().map(|r| r.total_games as i32).sum();
            ...
            if processed_count % 10 == 0 || processed_count == total_records {
                let elapsed: Duration = start_time.elapsed();
                let progress_percent: f64 = (processed_count as f64 / total_records as f64) * 100.0;
                println!(
        ...
        let updated_count: i32 = db.update_game_results(all_results)?;
        println!(
    ...
    fn initialize_model(
        ...
        let model_config: NnModelConfig = NnModelConfig::default();
        let model: NnModel<Autodiff<NdArray>> = if Path::new(model_save_path).exists() {
    ...
    fn process_training_batch(
        ...
        let batch_size: usize = batch_inputs.len();

        let mut flat_inputs: Vec<f32> = Vec::with_capacity(batch_size * input_dim);
        let mut flat_targets: Vec<f32> = Vec::with_capacity(batch_size * output_dim);
        ...
        let batch_input_tensor: Tensor<Autodiff<NdArray>, 2> =
            Tensor::<Autodiff<NdArray>, 1>::from_floats(flat_inputs.as_slice(), device)
                .reshape([batch_size, input_dim]);

        let batch_target_tensor: Tensor<Autodiff<NdArray>, 2> =
            Tensor::<Autodiff<NdArray>, 1>::from_floats(flat_targets.as_slice(), device)
                .reshape([batch_size, output_dim]);

        let predictions: Tensor<Autodiff<NdArray>, 2> = model.forward(batch_input_tensor);
        let loss: Tensor<Autodiff<NdArray>, 1> =
            Self::cross_entropy_loss_autodiff(&predictions, &batch_target_tensor);
        let loss_value: f32 = loss.clone().into_scalar();

        let grads: Gradients = loss.backward();
        let grads_params: GradientsParams = GradientsParams::from_grads(grads, &model);
        let updated_model: NnModel<Autodiff<NdArray>> = optim.step(current_lr, model, grads_params);
    ...
    fn update_progress_display(
        ...
        let progress_percent: f64 = (processed_samples as f64 / target_count as f64) * 100.0;
        let elapsed: Duration = epoch_start_time.elapsed();
        let elapsed_secs: f64 = elapsed.as_secs_f64();

        let remaining_samples: usize = target_count - processed_samples;
        let samples_per_sec: f64 = if elapsed_secs > 0.0 {
            processed_samples as f64 / elapsed_secs
        } else {
            0.0
        };
        let estimated_remaining_secs: f64 = if samples_per_sec > 0.0 {
    ...
    pub fn train_model(
        ...
        let total_count: usize = db.count_records_for_training(min_games)?;
        let target_count: usize = max_samples.unwrap_or(total_count).min(total_count);
        ...
        let mut model: NnModel<Autodiff<NdArray>> =
            Self::initialize_model(&model_save_path, &device)?;

        let optim_config: AdamConfig = AdamConfig::new();
        let mut optim: OptimizerAdaptor<Adam, NnModel<Autodiff<NdArray>>, Autodiff<NdArray>> =
            optim_config.init();
            ...
            loop {
                ...
                let results: Vec<_> = batch_records
                    ...
                    .map(|(board_sfen, white_wins, black_wins, total_games)| {
                        let board: Board = Board::from_sfen(board_sfen.clone());
                        let input: Vec<f32> = board.to_vector(None);

                        let total_games_f: f32 = *total_games as f32;
                        let white_win_rate: f32 = if *total_games > 0 {
                            *white_wins as f32 / total_games_f
                        } else {
                            0.5
                        };
                        let black_win_rate: f32 = if *total_games > 0 {
                            *black_wins as f32 / total_games_f
                        } else {
                            0.5
                        };
                        let draw_rate: f32 = if *total_games > 0 {
                ...
                let mut batch_inputs: Vec<Vec<f32>> = Vec::with_capacity(results.len());
                let mut batch_targets: Vec<Vec<f32>> = Vec::with_capacity(results.len());
    ...
    pub fn evaluate_position(
        ...
        let board_vector: Vec<f32> = board.to_vector(None);
        let prediction: Tensor<Autodiff<NdArray>, 1> = model.predict_single(board_vector);

        let white_win_rate: f32 = prediction.clone().slice([0..1]).into_scalar();
        let black_win_rate: f32 = prediction.clone().slice([1..2]).into_scalar();
        let draw_rate: f32 = prediction.slice([2..3]).into_scalar();
...
impl Evaluator for NeuralEvaluator {
    fn evaluate(&self, board: &Board, color: ColorType) -> f32 {
        let model_path: Option<&str> = self.model_path.as_deref();
        match self.evaluate_position(board, model_path) {
            ...
            Err(_) => {
                let simple_eval: SimpleEvaluator = SimpleEvaluator::new();
...
impl NeuralEvaluator {
    ...
    pub fn new_for_python(
        ...
        let db_type: Option<DatabaseType> = match (db_type_str, connection_string) {
